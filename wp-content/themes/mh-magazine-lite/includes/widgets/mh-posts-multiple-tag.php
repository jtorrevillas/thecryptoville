...
class mh_magazine_lite_posts_multiple_tag extends WP_Widget {
	function __construct() {
		parent::__construct(
			'mh_magazine_lite_posts_multiple_tag', esc_html_x('MH Posts Multiple Tag[lite]', 'widget name', 'mh-magazine-lite'),
			array(
				'classname' => 'mh_magazine_lite_posts_multiple_tag',
			...
			if ($widget_posts->have_posts()) :
				...
				echo '<div class="mh-posts-multiple-widget">' . "\n";
					while ($widget_posts->have_posts()) : $widget_posts->the_post();
						get_template_part('content', 'multiple-tag');
					endwhile;
					wp_reset_postdata();
				echo '</div>' . "\n";
			endif;
